...
const Level2Screen = () => {
  ...
  const handleNext = useCallback(async () => {
    ...
    if (currentState.idx === currentState.totalLines - 1) {
      ...
      // Restart recording after a short delay to allow avatar to start
      // You might want to wait for avatar to FINISH speaking, but for now we restart to keep flow
      setTimeout(async () => {
...
const styles = StyleSheet.create({
  ...
  waveformHidden: {
    height: 60,
    backgroundColor: '#F5F5F5', // Slightly different background to show it's inactive but visible
    paddingHorizontal: 15,
    opacity: 0.5, // Dimmed when not recording
  },
  waveformVisible: {
    height: 60,
    backgroundColor: 'white',
