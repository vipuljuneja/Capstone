...
export const createScenario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const scenario = await SimpleScenario.create(scenarioData);
...
export const getAllScenarios = async (req: Request, res: Response): Promise<void> => {
  try {
    const scenarios = await SimpleScenario.find().sort({ id: 1 });
...
export const getScenarioById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (/^[0-9a-fA-F]{24}$/.test(id)) {
      scenario = await SimpleScenario.findById(id);
    } else {
      // Try finding by custom id field (1, 2, 3)
      const numericId = parseInt(id);
      if (!isNaN(numericId)) {
        scenario = await SimpleScenario.findOne({ id: numericId });
      }
    }
...
export const updateScenario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let scenario = await SimpleScenario.findById(id);
    ...
    const updatedScenario = await SimpleScenario.findByIdAndUpdate(
      scenario._id,
      updateData,
...
export const updateScenarioQuestions = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const deleteScenario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await SimpleScenario.findByIdAndDelete(scenario._id);
...
export const initializeDefaultScenarios = async (req: Request, res: Response): Promise<void> => {
  try {
    // Check if scenarios already exist
    const existingScenarios = await SimpleScenario.find({});
    ...
    for (const scenarioData of scenarios) {
      await SimpleScenario.create(scenarioData);
    }
    
    const createdScenarios = await SimpleScenario.find({}).sort({ id: 1 });
